...
def load_and_preprocess_data(csv_path, seq_length=SEQ_LENGTH):
    ...
    # (Assumes 'Target Score' exists for chasing scenarios.)
    ...
    return X_seq, X_cum, X_ctx, y_target, ball_scaler, cum_scaler, ctx_scaler, chase_scaler, le, ball_features, cum_features, ctx_features
...
def train_model(csv_path, seq_length=SEQ_LENGTH, epochs=20, batch_size=32):
    X_seq, X_cum, X_ctx, y_target, ball_scaler, cum_scaler, ctx_scaler, chase_scaler, le, ball_features, cum_features, ctx_features = load_and_preprocess_data(csv_path, seq_length)
    ...
    model.fit([X_seq_train, X_cum_train, X_ctx_train[:, 0].reshape(-1, 1), X_ctx_train[:, 1:]],
...
if __name__ == "__main__":
    ...
    train_model(csv_path, seq_length=10, epochs=5, batch_size=32)
